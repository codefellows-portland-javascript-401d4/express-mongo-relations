...
router
  ...
  //serves all web articles with a specific property value
  .get('/search/:prop/:val', (req, res, next) => {
    //square bracket allows the variable to be key for an object
    WebArticle.find({[req.params.prop]: req.params.val})
      .then((data) => {
        res.send({message: 'Your articles have been found', data: data});
      })
      .catch((err) => {
        next(err);
      });
