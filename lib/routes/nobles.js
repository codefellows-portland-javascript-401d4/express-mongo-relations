...
//get all old nobles
    .get('/old', bodyParser, function (req, res, next) {
	Noble.find({alive: true})
        .where ('age').gt(55)
        .sort('-age')
        .select('name house age')
        .then(noble => res.send(noble ))
        .catch(next);
})

//get all nobles who died young
    .get('/died-young', bodyParser, function (req, res, next) {
	Noble.find({alive: false})
        .where ('age').lt(27)
        .sort('-age')
        .select('name house age')
        .then(noble => res.send(noble ))
